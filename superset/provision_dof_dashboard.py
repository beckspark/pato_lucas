...
BIG_NUMBER_CHART_NAME = "DOF -- Total anual"
...
def create_header_chart(client: SupersetClient, dataset_id: int) -> int:
    template = (
        ...
        "{{#with (lookup data 0)}}{{nombre_entidad}} — {{anio}}{{/with}}"
        "</h3>"
        "</div>"
    )
    params = {
...
def create_detail_table_chart(client: SupersetClient, dataset_id: int) -> int:
    params = {
        ...
        "viz_type": "table",
        "query_mode": "aggregate",
        "groupby": ["tipo_ramo", "fondo", "mes_nombre"],
        "metrics": [_metric_sum_monto()],
        "row_limit": 1000,
...
def _build_native_filters(ds_id: int) -> list[dict[str, Any]]:
    ...
    return [
        # 1. Anio
        build_filter(f_anio_id, "Anio", "anio", ds_id, multi=True, default_values=[2025]),
        # 2. Estado
        build_filter(f_estado_id, "Estado", "nombre_entidad", ds_id, default_values=["Jalisco"]),
        # 3. Tipo de ramo
        build_filter(f_tipo_ramo_id, "Tipo de ramo", "tipo_ramo", ds_id, multi=True),
        # 4. Fondo (cascada desde Tipo de ramo)
        build_filter(f_fondo_id, "Fondo", "fondo", ds_id, multi=True, cascade_from=f_tipo_ramo_id),
        # 5. Mes
        build_filter(f_mes_id, "Mes", "mes_nombre", ds_id, multi=True),
    ]
...
def main() -> None:
    ...
    try:
        ...
        big_number_id = create_big_number_chart(client, ds_id)
        ...
        position = build_position_json(
            ...
            [
                ...
                # Fila 1: Total anual (3 col) + Serie mensual (9 col), h=30
                [
                    ("bignumber", big_number_id, 3, 30, BIG_NUMBER_CHART_NAME),
                    ("line", line_id, 9, 30, LINE_CHART_NAME),
                ],
        ...
        print(
            ...
            f" big_number={big_number_id}, line={line_id},"
